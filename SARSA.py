...
def pSARSA(mA, fA, trn):
        ...
        if(ut[0] == 0 and ut[1] == 0 and p1.hold == 0 and p2.hold == 0):
            ...
            if trn%2 == 1:
                if mA.pickUP():
                    updateStatus()
                    ut2[v1]["pkUP"] = ut2[v1]["pkUP"] + (learningRate)*(13 + (discountRate)*nextAction(mA)-ut2[v1]["pkUP"])
                elif mA.DROP():
                    updateStatus()
                    ut2[v1]["drop"] = ut2[v1]["drop"] + (learningRate)*(13 + (discountRate)*nextAction(mA)-ut2[v1]["drop"]) 
                else:
                    chA = chooseAction(mA)
                    if chA == 0:
                        mA.moveUP()
                        ut2[v1]["up"] = ut2[v1]["up"] + (learningRate)*(-1 + (discountRate)*nextAction(mA)-ut2[v1]["up"])
                    if chA == 1:
                        mA.moveDOWN()
                        ut2[v1]["down"] = ut2[v1]["down"] + (learningRate)*(-1 + (discountRate)*nextAction(mA)-ut2[v1]["down"])
                    if chA == 2:
                        mA.moveLEFT()
                        ut2[v1]["left"] = ut2[v1]["left"] + (learningRate)*(-1 + (discountRate)*nextAction(mA)-ut2[v1]["left"])
                    if chA == 3:
                        mA.moveRIGHT()
                        ut2[v1]["right"] = ut2[v1]["right"] + (learningRate)*(-1 + (discountRate)*nextAction(mA)-ut2[v1]["right"])
                return 1
            elif trn%2 == 0:
                if fA.pickUP():
                    updateStatus()
                    ut2[v6]["pkUP"] = ut2[v6]["pkUP"] + (learningRate)*(13 + (discountRate)*nextAction(fA)-ut2[v6]["pkUP"])
                elif fA.DROP():
                    updateStatus()
                    ut2[v6]["drop"] = ut2[v6]["drop"] + (learningRate)*(13 + (discountRate)*nextAction(fA)-ut2[v6]["drop"]) 
                else:
                    chA = chooseAction(fA)
                    if chA == 0:
                        fA.moveUP()
                        ut2[v6]["up"] = ut2[v6]["up"] + (learningRate)*(-1 + (discountRate)*nextAction(fA)-ut2[v6]["up"])
                    if chA == 1:
                        fA.moveDOWN()
                        ut2[v6]["down"] = ut2[v6]["down"] + (learningRate)*(-1 + (discountRate)*nextAction(fA)-ut2[v6]["down"])
                    if chA == 2:
                        fA.moveLEFT()
                        ut2[v6]["left"] = ut2[v6]["left"] + (learningRate)*(-1 + (discountRate)*nextAction(fA)-ut2[v6]["left"])
                    if chA == 3:
                        fA.moveRIGHT()
                        ut2[v6]["right"] = ut2[v6]["right"] + (learningRate)*(-1 + (discountRate)*nextAction(fA)-ut2[v6]["right"])
                return 1
